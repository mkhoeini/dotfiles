...
(fn gen-key []
  ...
  (let [random-base64-str (->> 1e7
                               math.random
                               (string.format "%07d")
                               hs.base64.encode)]
    (string.sub random-base64-str 1 7)))
...
(fn emit [action data]
  ...
  Returns nil."
  (atom.reset! actions [action data]))
...
(fn ->enter [state action app-name]
...
(fn in-app->leave [state action app-name]
...
(fn launch-app [state action app-name]
...
(fn ->close [state action app-name]
...
(fn watch-actions [{: prev-state : next-state : action : effect : extra}]
...
(fn enter-app-effect [context]
  ...
  (when context.app
    ...
    (let [unbind-keys (bind-keys context.app.keys)]
      #(unbind-keys))))

(fn launch-app-effect [context]
...
(fn app-effect-handler [effect-map]
...
(local apps-effect
       (app-effect-handler
         {:enter-app-effect #(enter-app-effect $.context)
          :leave-app-effect #(do (-?> $.context.prev-app lifecycle.deactivate-app)
                                 nil)
          :launch-app-effect #(launch-app-effect $.context)
          :close-app-effect #(do (-?> $.context.prev-app lifecycle.close-app)
                                 nil)}))
...
(fn init [config]
  ...
  (let [active-app (active-app-name)
        ...
        template {:state {:current-state :general-app
                  ...
                  :states {:general-app {:enter-app ->enter
                                         :leave-app noop
