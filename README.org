...
Run this in the terminal. It will setup homebrew and emacs and run [[tldr]] elisp code for you.
...
** Setup script
:PROPERTIES:
:header-args: :tangle setup.sh :shebang "#!/bin/sh"
:END:
...
#+Name: install-deps
#+begin_src elisp :var formula-list=brew-formulas cask-list=brew-casks tap-list=brew-taps
  (let* ((tap-out (mapconcat (lambda (tap) (shell-command-to-string (format "echo brew tap '%s'" tap)))
                             tap-list))
         (formula-out (mapconcat (lambda (formula) (shell-command-to-string (format "echo brew install '%s'" formula)))
                                 formula-list))
         (cask-out (mapconcat (lambda (cask) (shell-command-to-string (format "echo brew install --cask '%s'" cask)))
                              cask-list)))
    (concat tap-out formula-out cask-out))
#+end_src
